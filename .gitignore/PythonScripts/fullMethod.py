...
#%%ANOVA on group TODO: tidy!!!
import statsmodels.api as sm
from statsmodels.formula.api import ols
import statsmodels.stats.multicomp

independentVars = ['feature','windowing']

oneWayANOVAs = {}
for group in significant_corrs_prop_df_clean.group.unique():
    data = significant_corrs_prop_df_clean[significant_corrs_prop_df_clean.group ==group]
    oneWayANOVAs[group] = {}
    for thing in independentVars:
        model = ols(f'coeff_proportion ~ C({thing})', data= data).fit()
        oneWayANOVAs[group][thing] =  model
        print(f"Overall model of ANOVA run for the {group} group on {thing} F({model.df_model: .0f},{model.df_resid: .0f}) = {model.fvalue: .3f}, p = {model.f_pvalue: .8f}")

data = significant_corrs_prop_df_clean[significant_corrs_prop_df_clean.group =='general']

featureVariance = ols('coeff_proportion ~ C(feature)', data= data).fit()
featureVariance = ols('coeff_proportion ~ C(windowing)', data=significant_corrs_prop_df_clean).fit()
print(f"Overall model F({featureVariance.df_model: .0f},{featureVariance.df_resid: .0f}) = {featureVariance.fvalue: .3f}, p = {featureVariance.f_pvalue: .8f}")


sessionVariance = ols('coeff_proportion ~ C(windowing)', data= data).fit()
print(f"Overall model F({sessionVariance.df_model: .0f},{sessionVariance.df_resid: .0f}) = {sessionVariance.fvalue: .3f}, p = {sessionVariance.f_pvalue: .8f}")

#empathyVariancePatSingle = ols('stdPat ~ C(coeff_proportion)', data=significant_corrs_empathy[[significant_corrs_empathy.group =='general',significant_corrs_empathy.feature =='melScale']]).fit()

empathyVariance = ols('stdPat ~ C(coeff_proportion)+C(windowing)', data=significant_corrs_empathy[(significant_corrs_empathy.group =='general') & (significant_corrs_empathy.feature =='melScale')]).fit()
print(f"Overall model F({empathyVariance.df_model: .0f},{empathyVariance.df_resid: .0f}) = {empathyVariance.fvalue: .3f}, p = {empathyVariance.f_pvalue: .8f}")


empathyVariance = ols('stdPat ~ C(coeff_proportion)', data=significant_corrs_empathy[(significant_corrs_empathy.group =='general') & (significant_corrs_empathy.feature =='melScale') & (significant_corrs_empathy.windowing =='slope')]).fit()
print(f"Overall model F({empathyVariance.df_model: .0f},{empathyVariance.df_resid: .0f}) = {empathyVariance.fvalue: .3f}, p = {empathyVariance.f_pvalue: .8f}")

test =stats.f_oneway(significant_corrs_prop_df['coeff_proportion'][significant_corrs_prop_df['group'] == 'correlationsF0'],
               significant_corrs_prop_df['coeff_proportion'][significant_corrs_prop_df['group'] == 'correlationsF0slope'],
             significant_corrs_prop_df['coeff_proportion'][significant_corrs_prop_df['group'] == 'correlationsF0std'],
             significant_corrs_prop_df['coeff_proportion'][significant_corrs_prop_df['group'] == 'correlationsLoud'],
             significant_corrs_prop_df['coeff_proportion'][significant_corrs_prop_df['group'] == 'correlationsLoudSlope'],
             significant_corrs_prop_df['coeff_proportion'][significant_corrs_prop_df['group'] == 'correlationsLoudStd'])

print(f"Overall model F({featureVariance.df_model: .0f},{featureVariance.df_resid: .0f}) = {featureVariance.fvalue: .3f}, p = {featureVariance.f_pvalue: .8f}")
#sessionVariance = ols('coefficient ~ C(session)*C(group)', data=significant_coeffs_df).fit() #WHY Error interpreting categorical data: all items must be hashable
aov_table = sm.stats.anova_lm(featureVariance, typ=2)

positiveVariance = ols('coeff_proportion ~ C(group)', data=positive_significant_corrs_prop_df).fit()
print(f"Overall model F({positiveVariance.df_model: .0f},{positiveVariance.df_resid: .0f}) = {positiveVariance.fvalue: .3f}, p = {positiveVariance.f_pvalue: .8f}")

empathyVariance = ols('coeff_proportion ~ C(pz)', data=significant_corrs_empathy).fit()
print(f"Overall model F({empathyVariance.df_model: .0f},{empathyVariance.df_resid: .0f}) = {empathyVariance.fvalue: .3f}, p = {empathyVariance.f_pvalue: .8f}")

anxietyVariance = ols('coeff_proportion ~ C(normScoring)', data=significant_corrs_empathy_anxiety).fit()
print(f"Overall model F({empathyVariance.df_model: .0f},{empathyVariance.df_resid: .0f}) = {empathyVariance.fvalue: .3f}, p = {empathyVariance.f_pvalue: .8f}")
#%%
from statsmodels.stats.multicomp import pairwise_tukeyhsd
# perform multiple pairwise comparison (Tukey HSD)
windowing = pairwise_tukeyhsd(endog=data['coeff_proportion'], groups=data['windowing'], alpha=0.05)
print(windowing.summary())

features = pairwise_tukeyhsd(endog=data['coeff_proportion'], groups=data['feature'], alpha=0.05)
print(features.summary())

#%%
formula = 'coeff_proportion ~ C(session) + C(windowing) + C(session):C(windowing)'
data = significant_corrs_prop_df_clean[significant_corrs_prop_df_clean.group =='general']
model = ols(formula, data).fit()
aov_table = sm.stats.anova_lm(model, typ=2)

#%%
formula = 'stdPat ~ C(coeff_proportion) + C(windowing) + C(coeff_proportion):C(windowing)'
data = significant_corrs_empathy[(significant_corrs_empathy.group =='general') & (significant_corrs_empathy.feature =='melScale')]
model = ols(formula, data).fit()
aov_table = sm.stats.anova_lm(model, typ=2)
#%% ANOVA  and t-test on real vs fake
fakeRealVariance = ols('coeff_proportion ~ C(type)', data=fake_real_df.dropna()).fit()
print(f"Overall model F({fakeRealVariance.df_model: .0f},{fakeRealVariance.df_resid: .0f}) = {fakeRealVariance.fvalue: .3f}, p = {fakeRealVariance.f_pvalue: .8f}")
t_test_fake_real = stats.ttest_ind(fake_real_df['coeff_proportion'].loc['real'], fake_real_df['coeff_proportion'].loc['fake'], nan_policy='omit')

#%% linear regression prep
# define the data/predictors as the pre-set feature names  
df = pd.DataFrame(significant_corrs_empathy_anxiety, columns=['group','session','coeff_proportion'])

# Put the target (housing value -- MEDV) in another DataFrame
emapthyPat = pd.DataFrame(significant_corrs_empathy_anxiety, columns=["pz"])
emapthyDoc = pd.DataFrame(significant_corrs_empathy_anxiety, columns=["ter"])
anxiety = pd.DataFrame(significant_corrs_empathy_anxiety, columns=['normScoring'])

#%%
X = df["coeff_proportion"]
y = anxiety["normScoring"]

model = sm.OLS(y, X).fit()
predictions = model.predict(X) # make the predictions by the model

modelTest = ols('y ~ C(df["coeff_proportion"])*C(df["group"])').fit()
